...
class OnUserRegisteredCallback(typing.Protocol):
    async def __call__(self, request: HttpRequest, user: User) -> None:
        ...


async def automatically_login_user(request: HttpRequest, user: User) -> None:
    await login(request, user)


async def create_free_subscription(request: HttpRequest, user: User) -> None:
...
async def create_user_organization(request: HttpRequest, user: User) -> None:
